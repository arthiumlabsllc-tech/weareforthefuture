...
export async function GET() {
  ...
  const members = await prisma.executiveBoardMember.findMany({ where: { deletedAt: null }, orderBy: [{ order: "asc" }, { name: "asc" }] });
...
export async function POST(request: NextRequest) {
  ...
  await prisma.auditLog.create({ data: { userId: auth.session.userId, action: "CREATE", entity: "ExecutiveBoardMember", entityId: member.id, after: { name: member.name } } });
